...
from src.infrastructure.mediator.middlewares.base import Middleware
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        if isinstance(request, Command):
            ...
            self._logger.log(self._level, "Publish %s event", type(request).__name__, extra={"occasion": request})
        else:
        ...
        if isinstance(request, Command):
            ...
            self._logger.log(
                self._level,
                "Query %s made. Result: %s",
                type(request).__name__,
                res,
                extra={"result": res},
            )
        elif isinstance(request, Event):
            self._logger.log(self._level, "Event %s published", type(request).__name__, extra={"occasion": request})
        else:
            self._logger.log(
                self._level,
                "Request %s executed. Result: %s",
                type(request).__name__,
                res,
                extra={"result": res},
            )
